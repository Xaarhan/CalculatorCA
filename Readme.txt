...
Все три  слоя сделаны в отдельных assembly. assembly Data, Presentation знают про assembly Domain, который не знает про другие


Слой Domain
  В самом базовом domain положил функцию сложения двух чисел с интерфейсом (int, int) : int
В Iteractors выделил: загрузку из лога, сохранение, и подсчет числа

Кроме того там же лежит интерфейс для gateway к репозитории, я не уверен что так правильно,
но если он будет в assembly Data то и assembly Domain придется знать про Data

Слой Data
  Тут только DataRepository и FileRepository. FileRepository умеет читать и писать txt файл на диск, а DataReposity 
это прослойка с простым интерфейсом, чтобы удобнее было потом подменять системы сохранения, например в базу данных вместо диска
...
Тут еще некоторые мои размышления
  ...
  В одних статьях в domain выделают только структуры данных, в других и самую базовую логику тоже.
